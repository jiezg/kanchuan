...
将文件（大小<20MB）编码为动态彩色二维码图像序列，通过摄像头实时解码还原文件，实现"看穿屏幕"的文件传输。
...
3. **图像解码**：点击"图像解码"，启动摄像头。将摄像头对准另一台设备上播放的 cimbar 动态图像，四角锚点变绿表示对齐成功，程序自动解码并保存文件。
4. **手机解码**：手机端使用 [CFC](https://github.com/sz3/cfc/releases) 扫描解码。将手机摄像头对准彩色图形矩阵条形码即可。
5. **注意事项**：大文件传输受单帧图像吞吐量限制时间会比较长，建议使用相对稳固无晃动的发送端（显示器）和接收端（支架、高拍仪）。
